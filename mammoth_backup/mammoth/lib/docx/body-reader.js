...
function BodyReader(options) {
    ...
    function readXmlElement(element) {
        if (element.type === "element") {
            ...
            if (handler) {
                ...
                var surroundWith = function(tagName, data) {
                    return '((' + tagName + '))' + data + '((/' + tagName + '))';
                }

                if (result.value.type == 'text') {
                    if (transformNextElement.highlight) {
                        result.value.value = surroundWith('highlight', result.value.value);
                        transformNextElement.highlight = false;
                    }
                    if (transformNextElement.underline) {
                        result.value.value = surroundWith('underline', result.value.value);
                        transformNextElement.underline = false;
                    }
                }

                if (result.value.type == 'runProperties') {
                    var props = result.value;
                    
                    if (props.isHighlight) transformNextElement.highlight = true;
                    if (props.isUnderline) transformNextElement.underline = true;
                }
    ...
    function readRunProperties(element) {
        var runPropertiesResult = readRunStyle(element).map(function(style) {
            var  result = {
                type: "runProperties",
                ...
                isBold: readBooleanElement(element.first("w:b"))
                    || style.styleId == 'Emphasis'
                    || style.styleId == 'Style13ptBold',
                isUnderline: readBooleanElement(element.first("w:u")) 
                    || style.styleId == 'StyleUnderline'
                    || style.styleId == 'Emphasis',
                isItalic: readBooleanElement(element.first("w:i")),
                isStrikethrough: readBooleanElement(element.first("w:strike")),
                isSmallCaps: readBooleanElement(element.first("w:smallCaps")),

                isHighlight: element.firstOrEmpty("w:highlight").attributes["w:val"]
            };
            return result;
        });
        
        return runPropertiesResult;
    }
    ...
    function readStyle(element, styleTagName, styleType, findStyleById) {
        ...
        if (styleElement) {
            ...
            if (styleId && styleId != 'StyleUnderline') {
